...
class Hangman():
    ...
    def check_guess(self,guess):
        ...
        if(guess in self.word):
            ...
            for i in range(len(self.word)):  # you could use enumerate to keep track of index
                # iterating through the word, and check if that element = to the guess letter, if so replace it
                if(self.word[i] == guess):
                    self.word_guessed[i] = guess
            # once we replaced all the underscores with the gueess, then we decrement the unique letters
            self.num_letters -= 1
